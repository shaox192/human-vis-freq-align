...
ATTACK_ALG = {
    "linf": {"pgd": LinfPGD, "fgsm": LinfFastGradientAttack},
    "l2": {"fgsm": L2FastGradientAttack, "deepfool": L2DeepFoolAttack},
}


def foolbox_attack(val_loader, model, device, epsilon, lp, attack_alg, kwargs={}):
    """
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        raw_advs, clipped_advs, success = attack(
            fmodel, images, target, epsilons=[epsilon]
        )

        robust_accuracy = 1 - success.float().mean(axis=-1)
        perturb_acc += robust_accuracy[0].item() * len(images)
    print(original_acc_sum)
